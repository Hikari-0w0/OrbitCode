...
- 分析 Agent 运行时，按 [运行评估工作流](docs/evals/README.md) 定位本地运行记录、按需读取证据，并使用其中的诊断模板。
- 用户要求评估或整理时，默认只诊断，不自动修改 Agent 行为、重跑开发任务或调用真实模型。
- 运行摘要、会话检查点和工作区文件是不同证据；不得把会话检查点视为文件快照，把最终回复视为任务验收通过。
