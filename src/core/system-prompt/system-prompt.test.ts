...
test("固定模块包含核心行为且不混入运行态标签", () => {
  ...
  assert.match(prompt, /多个独立文件.*多个 write_file 调用/u);
  assert.match(prompt, /不要把工具参数 JSON.*嵌入/u);
  assert.match(prompt, /find_files.*只支持.*\*.*\?.*\*\*/u);
  assert.match(prompt, /不要.*扫描 node_modules 或 \.next/u);
  assert.match(prompt, /不要猜测或拼接绝对路径/u);
  assert.match(prompt, /find_files 只能证明文件存在/u);
  assert.match(prompt, /可重复执行的长验证脚本/u);
  assert.match(prompt, /批量写入代码后优先运行一次类型检查或构建/u);
  ...
  assert.match(prompt, /工作区约定和已有项目脚本.*lint、test、typecheck、build/u);
  assert.match(prompt, /不得只选择已经通过的检查/u);
